...
public class SwiftMaskField: UITextField {
    ...
    public func applyFilter(textField: UITextField) {
        ...
        while(index != maskString.endIndex) {
            ...
            if("\(maskString[index])" == "N") { //Only number
                ...
                textWithMask += String(Array(text)[i])
                i += 1
                ...
                textWithMask += String(Array(text)[i]).uppercased()
                i += 1
                ...
                textWithMask += String(Array(text)[i]).lowercased()
                i += 1
    ...
    public func hasSpecialCharacter(searchTerm: String) -> Bool {
        guard let regex = try? NSRegularExpression(pattern: ".*[^A-Za-z0-9].*", options: NSRegularExpression.Options()) else {return false}
